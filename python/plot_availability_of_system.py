...
def availability_of_system(uptime_duration_distribution_mean,\
          uptime_duration_distribution_standard_deviation,\
          downtime_duration_distribution_mean,downtime_duration_distribution_standard_deviation,max_number_of_days):
  ...
  for hour_indx in range(max_number_of_days*hours_in_a_day): # =1:size(availability_array_per_hour,2)
    if ((uptime_hours_remaining<1) and (downtime_hours_remaining<1)):
      uptime_hours_remaining=gauss(uptime_duration_distribution_mean,uptime_duration_distribution_standard_deviation)
      if (uptime_hours_remaining<=1):
        uptime_hours_remaining=1
      downtime_hours_remaining=gauss(downtime_duration_distribution_mean,downtime_duration_distribution_standard_deviation)
      if (downtime_hours_remaining<=1):
        downtime_hours_remaining=1
    if ((uptime_hours_remaining>=1) and (downtime_hours_remaining>=1)): # system is up
